...
if [[ "${1}" == "" || "${1}" == "quotanizer" ]]; then
    docker build --progress plain --platform=linux/amd64 -t initialed85/cameranator-quotanizer:latest -f quotanizer/Dockerfile quotanizer # &
fi

if [[ "${1}" == "" || "${1}" == "segment-processor" ]]; then
    go build -v -o segment_processor ./cmd/segment_processor/main.go
    docker build --progress plain --platform=linux/amd64 -t initialed85/cameranator-segment-processor:latest -f docker/segment-processor/Dockerfile . # &
    rm -f segment_processor
fi

if [[ "${1}" == "" || "${1}" == "segment-generator" ]]; then
    go build -v -o segment_generator ./cmd/segment_generator/main.go
    docker build --progress plain --platform=linux/amd64 -t initialed85/cameranator-segment-generator:latest -f docker/segment-generator/Dockerfile . # &
    rm -f segment_generator
fi

if [[ "${1}" == "" || "${1}" == "event-pruner" ]]; then
    go build -v -o event_pruner ./cmd/event_pruner/main.go
    docker build --progress plain --platform=linux/amd64 -t initialed85/cameranator-event-pruner:latest -f docker/event-pruner/Dockerfile . # &
    rm -f event_pruner
fi

if [[ "${1}" == "" || "${1}" == "front-end" ]]; then
    docker build --progress plain --platform=linux/amd64 -t initialed85/cameranator-front-end:latest -f docker/front-end/Dockerfile . # &
fi

if [[ "${1}" == "" || "${1}" == "object-task-scheduler" ]]; then
    go build -v -o object_task_scheduler ./cmd/object_task_scheduler/main.go
    docker build --progress plain --platform=linux/amd64 -t kube-registry:5000/cameranator-object-task-scheduler:latest -f docker/object-task-scheduler/Dockerfile . # &
    rm -f object_task_scheduler
fi

if [[ "${1}" == "" || "${1}" == "object-task-worker" ]]; then
    docker build --progress plain --platform=linux/amd64 -t kube-registry:5000/cameranator-object-task-worker-nvidia-generic:latest -f docker/object-task-worker/Dockerfile.nvidia-generic . # &
    # docker build --progress plain --platform=linux/amd64 -t kube-registry:5000/cameranator-object-task-worker-nvidia-sm30:latest -f docker/object-task-worker/Dockerfile.nvidia-sm30 .       # &
    # docker build --progress plain --platform=linux/amd64 -t kube-registry:5000/cameranator-object-task-worker-amd-generic:latest -f docker/object-task-worker/Dockerfile.amd-generic .       # &
fi
